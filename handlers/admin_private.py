...
async def admin_command_go(message: types.Message, bot: Bot):
    if message.from_user.username:
        name_user = f'@{message.from_user.username}'
    else:
        name_user = message.from_user.full_name
    await message.answer(f'Здравствуйте {name_user}', reply_markup=simple_keyboard(buttons=['Добавить/Изменить меню', 'Добавить/Изменить частые вопросы', 'Добавить/Изменить баннер', 'Прочее'], sizes=(1,1,2)).as_markup(resize_keyboard=True, input_field_placeholder='Что вы хотите сделать?'))
    keyboard = None
...
async def add_product(message: types.Message, state: FSMContext, session: AsyncSession, bot: Bot):
    ...
    for num_category in range(len(bot.food_category)):
        ...
        if result and next_id < result[-1].id:
                next_id = result[-1].id
...
async def start_question(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    await state.set_state(AddQuestion.name)
    next_id = 0
    result = await orm_get_questions(session)
    if result:
        next_id = result[-1].id
    await state.update_data(id=next_id + 1)
