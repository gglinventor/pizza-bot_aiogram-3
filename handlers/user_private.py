...
async def add_to_cart(callback: types.CallbackQuery, callback_data: UserCallBack, session: AsyncSession):
    ...
    await orm_add_user(session=session, id=users[-1].id + 1, user_id=user.id, username=user_info, phone=None)
    await orm_add_to_cart(session=session, id=carts[-1].id + 1, user_id=user.id, product_id=callback_data.product_id)
