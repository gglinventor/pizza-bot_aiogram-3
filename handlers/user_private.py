...
async def add_to_cart(callback: types.CallbackQuery, callback_data: UserCallBack, session: AsyncSession):
    ...
    items_list = []
    for item in (users, carts):
        if item:
            items_list.append(item[-1].id)
        else:
            items_list.append(0)
    await orm_add_user(session=session, id=items_list[0] + 1, user_id=user.id, username=user_info, phone=None)
    await orm_add_to_cart(session=session, id=items_list[1] + 1, user_id=user.id, product_id=callback_data.product_id)
